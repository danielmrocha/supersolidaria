<?php
function gavias_kama_preprocess_node(&$variables) {
...
function gavias_kama_preprocess_node__article(&$variables) {
  $view_mode = $variables['view_mode']; 
  $allowed_view_modes = ['full']; 
  if(in_array($view_mode, $allowed_view_modes)) {
    $allowed_regions = ['related_content'];
    gavias_kama_add_regions_to_node($allowed_regions, $variables);
  }
}

function gavias_kama_preprocess_user(&$variables) {
  $allowed_regions = ['user_content'];
  gavias_kama_add_regions_to_node($allowed_regions, $variables);
  
}

function gavias_kama_preprocess_comment(&$variables){
   $date = $variables['comment']->getCreatedTime();
  $variables['created'] = date( 'j F Y', $date);
}

function gavias_kama_preprocess_field(&$variables, $hook) {
  $element = $variables['element'];
}  

function gavias_kama_preprocess_breadcrumb(&$variables){
